...
Works on Centos 8 and Let's Encrypt certbot version 1.3.0 with DNSimple's DNS API in a production environment, however YMMV. This script is compatible with all supported certbot domain API (AWS, DO, etc) options with minor adjustments. Further details are in the varying folder's README files.
