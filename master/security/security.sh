...
# TAR files..
TARG="ENCRYPTED-SSHKEYS-REPLACE.tar"
TAR="SSHKEYS-REPLACE.tar"
...
AUTOSSL="/home/$USER/auto-ssl/master/security"
...
exec 1> /home/$USER/auto-ssl/master/logs/security/$NOW-SECURITY.log 2>&1
...
#if grep -Fq "#CONFIG-LINE-REPLACE" $SSH/config
#then
        # code if found.
...
#	cp -v ./#SSHKEY-REPLACE $SSH/keys
#        chmod 600 $SSH/keys/#SSHKEY-REPLACE
#        printf "${GREEN}SSH GIT Config Set$s\n$s\n${NC}"
#fi

# Get $TAR
#printf "${RED}Aqcuiring TAR$s/n$s/n${NC}"
#rsync -azPi #SERVER-REPLACE:/#FILE-PATH/#SSHKEYS-REPLACE.tar.gpg . || -H break
...
if [ $? -eq 0 ]; then
	...
	bash /home/$USER/auto-ssl/master/audit.sh "$FAIL" "$ERRORPOINT"
fi
#shred -vfuz $TAR
